...
class InitialClientOnboarding(models.Model):
    ...
    #     """Create documents from the Due Diligence document lines"""
    #     documents_created = 0
    #     
    #     for rec in self:
    #         if not rec.document_required_type_ids:
    #             # Show user-friendly message if no documents to create
    #             return {
    #                 'type': 'ir.actions.client',
    #                 'tag': 'display_notification',
    #                 'params': {
    #                 'title': 'No Documents to Create',
    #                 'message': 'Please add documents in the Due Diligence tab first.',
    #                 'type': 'warning',
    #             }
    #         }
    #         
    #         for line in rec.document_required_type_ids:
    #             # Only create documents that haven't been moved yet and have required fields
    #             if not line.is_moved and line.document_id:
    #                 try:
    #                     # Create document in the documents app
    #                     vals = {
    #                         'name': line.name or f"{line.document_id.name} - {rec.partner_id.name}",
    #                         'folder_id': 15,  # Default folder - you may want to make this configurable
    ...
    #                         'datas': line.document if line.document else False,
    #                         'thumbnail': line.document if line.document else False,
    #                     }
    #                     
    #                     doc = self.env['documents.document'].sudo().create(vals)
    #                     
    #                     # Mark as moved to prevent duplicates
    #                     line.write({
    #                         'is_moved': True,
    #                         'is_ready': True
    #                     })
    ...
    #                     # Log success
    #                     rec.message_post(
    #                         body=f"Created document: {doc.name} (ID: {doc.id})"
    #                     )
    #                     
    #                 except Exception as e:
    #                     # Log error but continue with other documents
    #                     rec.message_post(
    #                         body=f"❌ Failed to create document {line.name}: {str(e)}"
    #                     )
    #                     continue
    #     
    ...
    #                 'message': f'Created {documents_created} document(s) successfully.',
    #                 'type': 'success',
    #             }
    #         }
    #     else:
    ...
    #                 'message': 'All documents have already been created or are missing required information.',
    #                 'type': 'info',
    #             }
    #         }
    ...
    def action_submit(self):
        for rec in self:
            ...
            if not user:
                # Fallback to current user if specific user not found
                user = self.env.user
            ...
            data = {
                ...
                'summary': _(' Assessment Validation : ' + rec.name + " -->  " + user_name),
                'note': _(' Assessment Validation : ' + rec.name + " -->  " + user_name),
        ...
        for rec in self:
            ...
            if rec.is_hide:
                ...
                data = {
                    ...
                    'activity_type_id': self.env['mail.activity.type'].sudo().search(
                        [('name', 'like', 'Compliance')]).id,
                    'summary': _(' Pending Compliance Officer Approval : ' + rec.name + " -->  " + user_name),
                    'note': _(' Pending Compliance Officer Approval : ' + rec.name + " -->  " + user_name),
                    'date_deadline': fields.Date.today() + timedelta(days=1),
                    'user_id': user.id
                }
                self.env['mail.activity'].sudo().create(data)
                ...
                data = {
                    ...
                    'activity_type_id': self.env['mail.activity.type'].sudo().search(
                        [('name', 'like', 'Compliance')]).id,
                    'summary': _(' Pending Management Approval : ' + rec.name + " -->  " + user_name),
                    'note': _(' Pending Management Approval : ' + rec.name + " -->  " + user_name),
                    'date_deadline': fields.Date.today() + timedelta(days=1),
                    'user_id': user.id
                }
                self.env['mail.activity'].sudo().create(data)
